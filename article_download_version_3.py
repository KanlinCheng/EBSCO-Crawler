...
本次更新改进了以下方面：
    - 根据期刊名称和时间段，自动创建文件夹；
    - 搜索结果超过50个（一个页面显示不全时），可以自动点击下一页，直至全部文件下载完成；
    - 文件下载完成后，在提示信息最后一行显示文件所在文件夹位置。
"""

"""
使用说明：
    1. 补全用户名和密码 (第56、57行）；
    2. 检查爬虫exe文件所在路径是否正确 (第59行）；
    3. 检查文件下载路径是否正确，视需求调整，输入的文件下载路径必须为已创建的文件夹 (第60行）；
    4. 将 article_name 替换为需要下载的期刊名 （第62行）；
    5. 按需求设置开始年月和结束年月 （第64-67行）；
    6. 程序会在用户指定的文件下载路径内，根据期刊名称和时间段，自动创建文件夹，并下载至此文件夹；
    7. 测试中发现 《Academy of Management Journal》 资源少，基本没有可下载资源，需要换数据库。
"""
...
def generate_new_path_name(journal_name, year_start, month_start, year_end, month_end):
    path_article_name = ""
...
if __name__ == "__main__":
    ...
    new_path_name = generate_new_path_name(article_name, year_start, month_start, year_end, month_end)
    ...
    # driver.find_element_by_xpath('//*[@id="_cluster_Journal%24journal+of+cleaner+production"]').click()    # Academy of Management Journal 资源少，基本没有可下载资源，需要换数据库
    ...
    while True:
        ...
        try:
            ...
            driver.find_element_by_xpath('//*[@id="ctl00_ctl00_MainContentArea_MainContentArea_bottomMultiPage_lnkNext"]').click()   # next page button
            time.sleep(10)
        except:
            break
